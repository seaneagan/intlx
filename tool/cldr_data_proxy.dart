...
class CldrDataProxy {
  ...
  Iterable<String> constrainLocales(Iterable<String> constraint) => 
    constraint.map((locale) => 
  ...
  transform(Map<String, dynamic> localeJsonMap) => 
    localeJsonMap.keys.fold(<String, String> {}, (map, locale) {
      print("transformed: ${transformJson(locale, localeJsonMap[locale])}");
      map[locale] = 
        json.stringify(transformJson(locale, localeJsonMap[locale]));
      return map;
    });
  ...
  void store(Map<String, String> localeJsonMap) {
    localeJsonMap.forEach((locale, json){
      var dataFile = 
        new File.fromPath(getLocaleDataFilePath(outputPath, locale));
